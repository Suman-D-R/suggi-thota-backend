...
export const sendOTP = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Try MessageCentral first, then fallback to custom OTP
    let verificationId: string | undefined;

    try {
      // Check if MessageCentral is configured
      if (envConfig.MESSAGE_CENTRAL_AUTH_KEY && envConfig.MESSAGE_CENTRAL_CUSTOMER_ID) {
        getLogger().info(`Attempting to send MessageCentral OTP to ${phoneNumber}`);

        // MessageCentral generates and sends OTP automatically
        // We get back a verificationId that we need to store
        const result = await smsService.sendOTPSMS(phoneNumber, '', 10); // OTP is generated by MessageCentral
        if (result) {
          verificationId = result;
          otpService.storeMessageCentralVerification(phoneNumber, verificationId);
          getLogger().info(`MessageCentral OTP sent to ${phoneNumber} via MessageCentral`);

          responseUtils.successResponse(
            res,
            'OTP sent successfully to your phone number via MessageCentral',
            responseData
          );
          return;
        }
      } else {
        getLogger().warn('MessageCentral not configured, falling back to custom OTP');
      }
    } catch (messageCentralError: any) {
      getLogger().warn('MessageCentral OTP failed, falling back to custom OTP:', messageCentralError.message);
      // Fall through to custom OTP service
    }

    // Fallback: Use custom OTP service if MessageCentral fails or not configured
    ...
    // Note: Custom OTP is already stored, no need to send SMS for custom OTP
    // (In development, OTP is included in response)
    getLogger().info(`Custom OTP generated for ${phoneNumber} (not sent via SMS)`);
  } catch (error) {
...
export const verifyOTP = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    getLogger().info(`Verify OTP: Stored OTP found: ${!!storedOTP}, isFirebase: ${storedOTP?.isFirebaseOTP}, isMessageCentral: ${storedOTP?.isMessageCentralOTP}`);
    ...
    // If MessageCentral OTP, verify using MessageCentral API
    if (storedOTP.isMessageCentralOTP && storedOTP.verificationId) {
      try {
        const isValid = await smsService.validateOTPViaMessageCentral(phoneNumber, otp, storedOTP.verificationId);
        if (!isValid) {
          otpService.deleteOTP(phoneNumber);
          responseUtils.unauthorizedResponse(res, 'Invalid OTP. Please check and try again.');
          return;
        }
        getLogger().info(`MessageCentral OTP verified for ${phoneNumber}`);
      } catch (messageCentralError: any) {
        getLogger().error('MessageCentral OTP verification failed:', messageCentralError);
        otpService.deleteOTP(phoneNumber);
        responseUtils.unauthorizedResponse(res, messageCentralError.message || 'Invalid OTP. Please check and try again.');
        return;
      }
    } else if (storedOTP.isFirebaseOTP && storedOTP.sessionInfo) {
    ...
    if (cartData && cartData.storeId && cartData.items && Array.isArray(cartData.items) && cartData.items.length > 0) {
      ...
      try {
        ...
        if (store && store.isActive) {
          ...
          if (!cart) {
            cart = new Cart({ userId: user._id, storeId, items: [], issues: [] });
          }
