...
export const sendOTP = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    getLogger().info(`═══════════════════════════════════════════════════════════`);
    getLogger().info(`📱 OTP GENERATED FOR TESTING`);
    getLogger().info(`Phone Number: ${phoneNumber}`);
    getLogger().info(`OTP: ${otp}`);
    getLogger().info(`Expires in: 10 minutes`);
    getLogger().info(`═══════════════════════════════════════════════════════════`);
    ...
    responseUtils.successResponse(
      res,
      'OTP sent successfully. Please use the OTP from the response.',
      responseData
    );

    getLogger().info(`OTP generated and returned in response for ${phoneNumber}`);
  } catch (error) {
...
export const verifyOTP = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (storedOTP.isFirebaseOTP && storedOTP.sessionInfo) {
      ...
      getLogger().info(`OTP verified successfully for ${phoneNumber}`);
    }
